package tl.bnctl.banking.ui.banking.fragments.transfers.create.screens.account
...
open class AccountTransferFragment : BaseTransferFragment() {
    ...
    override fun populateViewFromTemplate(template: Template) {
        ...
        if (destinationAccountViewModel.email.value == null) {
            destinationAccountViewModel.setEmail(template.email ?: "")
        }
    ...
    override fun onValidationFailure(transferEnd: TransferEnd) {
        requireView().findNavController()
            .navigate(
                AccountTransferFragmentDirections.actionNavFragmentAccountTransferToNavFragmentTransferEnd(
                    transferEnd
                )
            )
    }
    ...
    override fun onValidationSuccess(summary: TransferSummary) {
        requireView().findNavController()
            .navigate(
                AccountTransferFragmentDirections.actionNavFragmentAccountTransferToNavFragmentTransferSummary(
                    summary
                )
            )
    }
    ...
    override fun isInputValid(): Boolean {
        destinationAccountViewModel.validateDestinationAccountNumber()
    ...
    protected fun shouldValidateIban(): Boolean {
        val transferType = destinationAccountViewModel.transferType.value
        return transferType == TransferType.INTERBANK || transferType == TransferType.INTERNATIONAL
    }
    ...
    override fun generateSummary(
        ...
        summary.to =
            Payee(
                Beneficiary(destinationAccountViewModel.beneficiaryName.value!!),
    ...
    protected open fun generateNewPayee(summary: TransferSummary): TemplateRequest {
        val bank = destinationAccountViewModel.bank
        return TemplateRequest(
            "bank",
