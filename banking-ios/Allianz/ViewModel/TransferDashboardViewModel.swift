...
class TransferDashboardViewModel: ObservableObject {
    ...
    func makeTransfer(transfer: Transfer) {
        ...
        TransferService().createTransfer(params: transfer) { [self] error, result, fallback in
            if let error = error {
    ...
    func validateTransfer(transfer:Transfer) async throws -> (TransferValidateResponse, Bool) {
        do {
            async let transferValidation = try TransferService().validateTransfer(transfer: transfer)
            async let isEoD = try EndOfDayService().checkEndOfDay()
            
            return try await (transferValidation, isEoD)
        } catch {
            throw error
        }
