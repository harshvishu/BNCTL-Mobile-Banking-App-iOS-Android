...
class BasicRequest: NSObject, URLSessionDelegate, NetworkReachabilityProtocol {
    ...
    func request<Body: Encodable> (
        url: ServiceType,
        method: HTTPMethod,
        encoding: HTTPEncoding = .jsonEncoded,
        parameters: Body? = nil,
        ...
        setContentType(encoding, &request)
        ...
        if let body = parameters{
            ...
            let httpBody = encodeHttpBody(body, usingEncoding: encoding)
...
extension BasicRequest {
    ...
    private func setContentType(_ encoding: HTTPEncoding, _ request: inout URLRequest) {
        switch encoding {
        case .urlFormEncoded:
