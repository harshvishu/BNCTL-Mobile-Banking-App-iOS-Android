...
struct CardsDashboard: View {
    ...
    @EnvironmentObject var pushProvisioningStatusModel:PushPrivisioningStatusViewModel
    ...
    @State var isShowingRequestCardView: Bool = false
    
    @State var cardForProvisioning:Card?
    ...
    var body: some View {
        ...
        .onAppear(perform: {           
            model.fetchCards()
            Task {
                await pushProvisioningStatusModel.checkStatus()
                
                if let state = pushProvisioningStatusModel.state,
                   state.isEnabled,
                   state.launchDate < Date()
                {
                    model.fetchCards(forTokenization: .all)
                } else {
                    model.fetchCards(forTokenization: .none)
                }
            }
        })
        .onDisappear(perform: {
    ...
    private var cardsView: some View {
                ...
                VStack {
                    ...
                    ScrollView {
                        VStack(spacing: 0) {
                            ...
                            //Walllet button
                            if let pushProvisioningState = pushProvisioningStatusModel.state,
                               pushProvisioningState.isEnabled,
                               pushProvisioningState.launchDate < Date(),
                               let card = model.cardToTokenization[currentCard.cardId],
                               card.state == .ready
                            {
                                Button {
                                    model.startTokenization(cardId: currentCard.cardId)
                                    cardForProvisioning = currentCard
                                } label: {
                                    AddToWalletButton()
        ...
        .hiddenNavigationBarStyle()
        .showTabBar()
        .sheet(item: $cardForProvisioning) { card in
            if let configuration = model.cardToTokenization[card.cardId]?.data,
               let _ = configuration.addPaymentPassRequestConfiguration
            {
                AddToWalletView(
                    tokenizationResponseData: configuration
                ) { pass, error in
                    let ctd = model.cardToTokenization[card.cardId]
                    if let _ = pass {
                        model.cardToTokenization[card.cardId] = ctd?.moveState(
                            newState: .added
                        )
                        
                        model.fetchCards(forTokenization: .card(card.cardId))
                        
                        if hasTokanisedCards == false {
                            hasTokanisedCards = true
                        }
                    } else if let _ = error {
                        model.cardToTokenization[card.cardId] = ctd?.moveState(
                            newState: .ready
                    ...
                    cardForProvisioning = nil
                }
            } else {
                HStack {
                    ProgressView()
                        .padding()
                    Text("card_waiting_tokenization")
                }
                    .onAppear {
                        model.fetchCards(forTokenization: .card(card.cardId))
                    }
