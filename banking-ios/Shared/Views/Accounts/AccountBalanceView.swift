            ...
            ZStack {
                ...
                VStack(alignment: .leading) {
                    ...
                    VStack(alignment: .leading, spacing: 28) {
                        if let openingBalance = account.balance.opening {
                            DetailsRow(
                                title: "account_details_label_opening_balance",
                                value: "\(openingBalance.toCurrencyFormatter()) \(account.currency)",
                                canCopy: false)
                        }
                        DetailsRow(
                        ...
                        DetailsRow(
                            ...
                            canCopy: false)
                        if let overdraft = account.balance.opening {
                            DetailsRow(
                                title: "account_details_label_overdraft",
                                value: "\(overdraft.toCurrencyFormatter()) \(account.currency)",
                                canCopy: false)
                        }
                        if let blockedBalance = account.balance.opening {
                            DetailsRow(
                                title: "account_details_label_blocked_amount",
                                value: "\(blockedBalance.toCurrencyFormatter()) \(account.currency)",
                                canCopy: false)
                        }
                    }
                    .padding(.vertical, 20)
